...
async def users_query_exe(unique_string, date_to, date_from):
    cur.execute(f"""SELECT {unique_string} date,
                    ROW_NUMBER() OVER (PARTITION BY users.userid) AS count 
                    FROM actions
                    JOIN users 
...
@app.get("/api/actions")
async def actions():
    response_list = await action_query_exe()
    response_dict = {"data": {}}
    for i in range(len(response_list)):
...
@app.get("/api/usage")
async def usage():
    query_result = await action_query_exe()

    # group by date
    response_list = []
    j = 0
    # response_list.append([query_result[0][0], query_result[0][1], []])
    for i in range(len(query_result)):
        if not (query_result[i-1][0] == query_result[i][0] and i != 0):
            response_list.append([query_result[i][0], j, []])
            j += 1
